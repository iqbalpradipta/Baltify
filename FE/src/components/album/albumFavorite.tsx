...
import ListAlbumFav from './listAlbumFav';
import { useState } from 'react';
import ButtonPlay from '../buttonPlay/buttonPlay';

function AlbumFavorite() {
  const num = [0,1,2]

  return (
    <>
      <Grid container spacing={{ xs: 2, md: 3 }} direction="row" justifyContent="center" alignItems="center" padding='10px'>
        <Grid item xs={6}>
          <Stack spacing={2}>
          {num.map(() => (
            <Box sx={{border: 1 ,boxShadow: 3, borderRadius: 3 ,backgroundColor: '#717171', width: '95%', height: '70px', display: 'flex', flexDirection: 'row' }}>
            <Box
                  sx={{
                    borderRadius: '10px',
                    display: 'flex',
                ...
                <Box sx={{position: 'absolute', left: 100,mt: 2}}>Rei Sarah</Box>
                <Box sx={{position: 'absolute', left: 100, mt: 5}}>Disoriented</Box>
              <Box position='absolute' left='1210px'>
                <ButtonPlay />
              </Box>
            </Box>
           ))}
